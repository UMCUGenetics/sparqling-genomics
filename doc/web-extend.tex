...
\label{sec:static-pages}
  ...
  When a visitor of \program{sg-web} is not logged in, static pages
  ...
  The following example adds a page called ``About us'' to \program{sg-web}:
...
\end{siderules}
  ...
  To let \program{sg-web} display this page, save the file to
  \file{/opt/pages/www/pages/static/about-us.scm}, and set
  \t{GUILE\_LOAD\_PATH} to \file{/opt/pages} before invoking \program{sg-web}.
...
\label{sec:sweave-reports}
  ...
  When \program{sg-web} is compiled with R support it will look for
