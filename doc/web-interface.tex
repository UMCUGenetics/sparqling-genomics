...
<web-interface>
  ...
  <port>8080</port>
  <static-pages>
    <page home="1">welcome</page>
    <page>about</page>
  </static-pages>
  <system-state-graph>sg-web://state</system-state-graph>
  <developer-mode>0</developer-mode>
  <backtrace-on-error>0</backstrace-on-error>
  <debug-log>/var/log/sg-web.log</debug-log>
  <error-log>/var/log/sg-web.errors</error-log>
  <system-connection>
...
\subsection{Static pages}
  ...
  the \t{/login} page.  This can be changed by specifying static pages.  See
  section \refer{sec:static-pages} to read about how to add static pages to
  the \program{sg-web} instance.

  Menu items of the static pages can be configured using the \t{<static-pages>}
  configuration, where each page gets its own \t{<page>} entry.  Which of
  the static pages becomes the landing page can be configured with the
  \t{home="1"} argument.

\subsection{System state graph}

  The state displayed by \program{sg-web} is stored in the
  \t{system-connection}'s RDF store.  All triplets are stored in a single
  graph configured with the \t{system-state-graph} configuration option.

  Changing the \t{system-state-graph} makes it look like a completely empty
  system.  One can revert to the previous state by using the previous
  \t{system-state-graph}.
...
\label{sec:logging}
  ...
  non-critical messages), or as options in the configuration file.  The
  latter takes precedence over the former.

  Backtraces help the developer to get an idea about how something might've
  gone wrong.  It displays the approximate location in the source code
  that caused an unrecoverable error.  Backstraces can get long and verbose.
  Therefore we don't display them by default.  To enable reporting backtraces
  in the error log, set the `\t{backtrace-on-error}' option to \t{1}.
